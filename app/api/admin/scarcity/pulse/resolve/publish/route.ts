...
import { publishMetalPulseResolutionEvidence } from "@/lib/metal-pulse-evidence-store";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    headers = secureMutation(request, {
      key: "scarcity-pulse-resolution-publish",
      limit: 12,
      windowMs: 3_600_000,
    }, 32_000).headers;
    ...
    const published = await publishMetalPulseResolutionEvidence({
      ...
      artifacts: [fetched.artifacts.opening, fetched.artifacts.closing].filter((artifact) => artifact !== null),
    });
    return NextResponse.json({
      ...
      report: prepared.report,
      resolveInstruction: published.signable ? serializeMetalPulseInstruction(prepared.resolveInstruction) : null,
      warning: published.signable
        ? "Evidence is persisted and the instruction is ready for independent review. It has not been signed or submitted."
        : "Evidence is incomplete. No resolver instruction is exposed for signing; manual source review is required.",
    }, { headers: { ...headers, "cache-control": "private, no-store" } });
  } catch (error) {
    const security = apiSecurityError(error);
    return NextResponse.json(
      { error: security?.message ?? (error instanceof Error ? error.message : "Metal Pulse resolution evidence could not be published.") },
