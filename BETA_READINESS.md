...
- `HEDGENTS_MAX_SOL_DEBIT_LAMPORTS` is mandatory and caps the taker's exact net SOL debit (network fee and any rent included). It is a single flat ceiling, not a per-route rent-and-fee derivation, so any headroom above the observed debit is spendable by the route. The example `10000000` (0.01 SOL) is far too loose for launch: read `takerSolDebitLamports` from the passing simulation matrix and set the lowest value that clears it. Sampled 2026-08-11 over 64 zero-spend PAXG/USDC routes at $10/$25/$50/$100, the observed debit was `2197677`–`2224604` for a first buy (Token-2022 ATA rent dominates) and `5222`–`35425` for a sell or a repeat buy. Production is set to `2600000`.
...
- Production requires `HEDGENTS_SOLANA_PROGRAM_ALLOWLIST`. Every program the route invokes must appear in it; one unreviewed program fails the trade closed with 503 before a signature is requested. Populate it only from program IDs read out of reviewed, passing zero-spend routes.
- The review unit is the **program**, not the program set. The fingerprint hashes the whole sorted set, so it changes with the venue Jupiter picks and again with whether the taker already holds the product: sampling on 2026-08-11 produced five legitimate fingerprints for one PAXG/USDC pair, including a distinct one for a returning tester whose buy no longer creates an associated token account. Pinning fingerprints would have rejected that tester's second trade. `HEDGENTS_SOLANA_PROGRAM_FINGERPRINT_ALLOWLIST` therefore remains supported but optional: set it only to additionally pin exact program sets, and expect to maintain it per venue and per first-versus-repeat trade.
- The launch program set for PAXG is the Solana platform programs (System, ComputeBudget, Associated Token, SPL Token, Token-2022, Memo), the Jupiter aggregator, and the venues PAXG actually routes through: Orca Whirlpool, plus the private market-maker DEXes ZeroFi and GoonFi, which have no user interface and route exclusively through Jupiter. Removing either private venue is safe but sharply narrows PAXG liquidity. Run `npm run sample:envelope` across several windows before an invite wave, and treat a tester-reported "has not passed the operator program review" as a review task, not a bug.
- The tester must declare a two-letter residence code and attest issuer eligibility before order assembly. The server repeats the check at submission. xStocks mirrors its published restricted-country policy in both browser and server checks; an operator allowlist can narrow that scope. Ondo securities remain fail-closed without an operator-approved country allowlist.
...
Holder discovery accepts a public wallet only when the large token account **is** that wallet's canonical associated token account for the mint. Jupiter builds every swap against the taker's ATA, so a whale that custodies through a non-ATA account answers "Insufficient funds" and would otherwise be recorded as a missing route rather than as a bad taker choice. Two provider limits are worth knowing before reading a failed run: Alchemy answers `getTokenLargestAccounts` for metal mints but times out on USDC, so set `HEDGENTS_SIMULATION_WALLET` explicitly when Alchemy is the primary; and `solana-rpc.publicnode.com` blocks `getTokenLargestAccounts` outright, which makes it usable as the second simulation host but not as a general fallback.

Sample the two guard settings that a single matrix run cannot establish, across several windows and trade sizes:

```bash
JUPITER_API_KEY=... \
HEDGENTS_SOLANA_MAINNET_RPC_URLS=https://provider-a...,https://provider-b... \
npm run sample:envelope -- gold-paxg 3
```

It repeats the real order and simulation path at $10/$25/$50/$100, then reports the observed SOL-debit range with a recommended cap and every distinct fingerprint with its programs and router. It signs and submits nothing.
...
7. Review every product’s current issuer terms and eligible jurisdictions. Keep the browser gate and server policy in sync; never rely on the browser as the enforcement boundary.
