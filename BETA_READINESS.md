...
- The fingerprint is a hash of the transaction's whole sorted program set, so it changes with the venue Jupiter picks **and** with whether the taker already holds the product. One PAXG/USDC pair therefore has several legitimate fingerprints. Sampling on 2026-08-11 produced five: a first buy through ZeroFi (`b0e322d2…`) and through Orca Whirlpool (`c4a3e23e…`), the same buy for a wallet that already holds PAXG (`c61460ba…`), and sells through GoonFi (`4d5bee0c…`) and GoonFi plus Whirlpool (`7a13ff55…`). Expect testers to hit an unlisted fingerprint whenever routing moves to a venue that has not been reviewed. That failure is safe (503, nothing signed), but recovering from it needs an operator review plus an env change plus a redeploy, so run `npm run sample:envelope` across several windows before an invite wave and treat any tester-reported "has not passed the operator canary review" as a review task, not a bug.
